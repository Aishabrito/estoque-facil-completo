...
export default function Entradas() {
  ...
  const totalItens = entradas.reduce((acc, item) => acc + item.qtd, 0);

  const filtradas = entradas.filter(item =>
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pt-20 md:pt-8 w-full">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <ArrowUpCircle className="text-emerald-600" /> Histórico de Entradas
            </h1>
            <p className="text-sm text-gray-500 mt-1">Todas as entradas registradas no estoque</p>
          </div>
          <button
            onClick={() => exportarCSV(filtradas, 'entradas.csv')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div className="p-3 bg-emerald-50 rounded-xl"><TrendingUp size={22} className="text-emerald-600" /></div>
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total de Registros</p>
              <p className="text-2xl font-black text-gray-900">{entradas.length}</p>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div className="p-3 bg-blue-50 rounded-xl"><ArrowUpCircle size={22} className="text-blue-600" /></div>
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Unidades Entrada</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Última Entrada</p>
              <p className="text-lg font-black text-gray-900">
                {entradas[0] ? new Date(entradas[0].data).toLocaleDateString('pt-BR') : '-'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-4">
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50/50 text-sm"
              value={searchTerm}
                ...
                {loading ? (
                  ...
                  filtradas.map((item) => (
                    <tr key={item.id} className="hover:bg-emerald-50/20 transition-colors">
                      ...
                      <td className="p-5">
                        <span className="inline-flex items-center gap-1 text-emerald-700 font-black bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-full text-sm">
                          +{item.qtd} un.
                        </span>
                      </td>
                      ...
                      <td className="p-5">
                        {item.usuario ? (
                          <div className="flex items-center gap-2">
                            <div className="h-7 w-7 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-black">
