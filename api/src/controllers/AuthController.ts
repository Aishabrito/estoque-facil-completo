...
export default {
  ...
  async atualizarPerfil(req: Request, res: Response): Promise<Response> {
    try {
      const { nome, email, senhaAtual, novaSenha } = req.body as {
        nome: string;
        email: string;
        senhaAtual?: string;
        novaSenha?: string;
  ...
  // LISTAR EQUIPE
  async listarUsuarios(_req: Request, res: Response): Promise<Response> {
    try {
  ...
  async atualizarUsuario(req: Request, res: Response): Promise<Response> {
    const { id } = req.params;
    const { nome, email, isAdmin } = req.body as { nome: string; email: string; isAdmin: boolean };
